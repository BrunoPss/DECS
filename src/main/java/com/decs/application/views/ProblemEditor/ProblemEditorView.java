...
import com.decs.application.views.notifications.SuccessNotification;
...
public class ProblemEditorView extends Composite<VerticalLayout> {
    ...
    private void saveProblem(SaveEvent event) {
        ...
        try {
            if (!(event.getSource().getProblemCode().isBlank() || event.getSource().getProblemFullName().isBlank() || event.getSource().getProblemType().isBlank())) {
                ...
                SuccessNotification.showSuccessNotification("Problem Created!");
            }
            else {
                ErrorNotification.showErrorNotification("Please fill all information!");
            }
        } catch (Exception e) {
            System.err.println("Exception while saving new problem");
            ErrorNotification.showErrorNotification("Error while saving the problem!");
        }
    }
